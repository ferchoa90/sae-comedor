...
class Sistema_proveedores extends Component
{
    private const MODULO='SISTEMA - PROVEEDORES';

    public function getProveedoreses($tipo,$array=true,$orderby,$limit,$all=true)
    {
        if ($all){
            $response= Proveedores::find()->where(["isDeleted"=>0])->all();
        }else{
            $response= Proveedores::find()->where(["isDeleted"=>0])->one();
        }
    }

    public function getProveedores($id,$condicion=NULL,$itemsret=NULL)
    {
        $result=array();
        if ($id){
            $result= Proveedores::find()->where(["id"=>$id])->one();
            if ($result)
            {
    ...
    {
        $model = Proveedores::find()->where(["isDeleted" =>0,"estatus"=>"ACTIVO"])->orderBy(["nombre" => SORT_ASC])->all();
        //var_dump($model);
        $modelArray=array();
        $cont=0;
        foreach ($model as $key => $value) {
            if ($cont==0){ $modelArray[$cont]["value"]="Seleccione un proveedor"; $modelArray[$cont]["id"]=-1; $cont++; }
    ...
    public function Editar($data)
    {
        ...
        if ($data):
            //$data = $usuario;
            $dataModel= Proveedores::find()->where(["id"=>$data['id']])->one();
    ...
    public function Eliminar($id)
    {
        ...
        if ($id):
            //$data = $usuario;
            $dataModel= Proveedores::find()->where(["id"=>$id])->one();
