...
class Inventario_presentacion extends Component
{
    const MODULO='PRESENTACION';
    public function getPresentaciones($tipo,$array=true,$orderby,$limit,$all=true)
    {
        if ($all){
            $asiento= Presentacion::find()->where(["isDeleted"=>0])->orderBy(["nombre"=>SORT_DESC])->all();
        }else{
            $asiento= Presentacion::find()->where(["isDeleted"=>0])->one();
        }
    }

    public function getPresentacion($id,$condicion=NULL,$itemsret=NULL)
    {
        $result=array();
        if ($id){
            $result= Presentacion::find()->where(["id"=>$id])->one();
            if ($result)
            {
    ...
    {
        $model = Presentacion::find()->where(["isDeleted"=>0,"estatus"=>"ACTIVO"])->orderBy(["nombre" => SORT_DESC])->all();
        //var_dump($model);
        $modelArray=array();
        $cont=0;
    ...
    public function Editar($data)
    {
        ...
        $result=false;
        if ($data):
            $model= Presentacion::find()->where(["id"=>$data['id']])->one();
            if ($data['id']){
