...
class Inventario_colores extends Component
{
    const MODULO='COLOR';
    public function getColores($tipo,$array=true,$orderby,$limit,$all=true)
    {
        if ($all){
            $asiento= Color::find()->where(["isDeleted"=>0])->orderBy(["nombre"=>SORT_DESC])->all();
        }else{
            $asiento= Color::find()->where(["isDeleted"=>0])->one();
        }
    }

    public function getColor($id,$condicion=NULL,$itemsret=NULL)
    {
        $result=array();
        if ($id){
            $result= Color::find()->where(["id"=>$id])->one();
            if ($result)
            {
    ...
    {
        $model = Color::find()->where(["isDeleted"=>0,"estatus"=>"ACTIVO"])->orderBy(["nombre" => SORT_DESC])->all();
        //var_dump($model);
        $modelArray=array();
        $cont=0;
    ...
    public function Editar($data)
    {
        ...
        $result=false;
        if ($data):
            $model= Color::find()->where(["id"=>$data['id']])->one();
            if ($data['id']){
