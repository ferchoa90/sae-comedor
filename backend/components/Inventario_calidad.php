...
class Inventario_calidad extends Component
{
    const MODULO='CALIDAD';
    public function getCalidades($tipo,$array=true,$orderby,$limit,$all=true)
    {
        if ($all){
            $asiento= Calidad::find()->where(["isDeleted"=>0])->orderBy(["nombre"=>SORT_DESC])->all();
        }else{
            $asiento= Calidad::find()->where(["isDeleted"=>0])->one();
        }
    }

    public function getCalidad($id,$condicion=NULL,$itemsret=NULL)
    {
        $result=array();
        if ($id){
            $result= Calidad::find()->where(["id"=>$id])->one();
            if ($result)
            {
    ...
    {
        $model = Calidad::find()->where(["isDeleted"=>0,"estatus"=>"ACTIVO"])->orderBy(["nombre" => SORT_DESC])->all();
        //var_dump($model);
        $modelArray=array();
        $cont=0;
    ...
    public function Editar($data)
    {
        ...
        $result=false;
        if ($data):
            $model= Calidad::find()->where(["id"=>$data['id']])->one();
            if ($data['id']){
