...
class Inventario_marca extends Component
{
    private const MODULO='INVENTARIO - MARCA';

    public function getMarcaes($tipo,$array=true,$orderby,$limit,$all=true)
    {
        if ($all){
            $response= Marca::find()->where(["isDeleted"=>0])->all();
        }else{
            $response= Marca::find()->where(["isDeleted"=>0])->one();
        }
    }

    public function getMarca($id,$condicion=NULL,$itemsret=NULL)
    {
        $result=array();
        if ($id){
            $result= Marca::find()->where(["id"=>$id])->one();
            if ($result)
            {
    ...
    {
        $model = Marca::find()->where(["isDeleted" =>0,"estatus"=>"ACTIVO"])->orderBy(["nombre" => SORT_ASC])->all();
        //var_dump($model);
        $modelArray=array();
        $cont=0;
        foreach ($model as $key => $value) {
            if ($cont==0){ $modelArray[$cont]["value"]="Seleccione una marca"; $modelArray[$cont]["id"]=-1; $cont++; }
    ...
    public function Editar($data)
    {
        ...
        if ($data):
            //$data = $usuario;
            $dataModel= Marca::find()->where(["id"=>$data['id']])->one();
    ...
    public function Eliminar($id)
    {
        ...
        if ($id):
            //$data = $usuario;
            $dataModel= Marca::find()->where(["id"=>$id])->one();
