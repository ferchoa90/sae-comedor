...
&nbsp;&nbsp;
                ...
                <div class="card-body">
                  ...
                  <div class="pull-right " style="clear: both;"></div>
                  <div id="dvrecargo" class="pull-right " style="clear: both; display:none !important;">
...
    delay: 500,
  ...
  $('#producto').keypress(function(e) {
   ...
   function obtenerProductoId(){
    console.log('obtenerP Array ')
    //$('#idproducto').val(0)
    $.ajax({
     url:\"productoid\",
     method:\"POST\",
     data:{id:".$idorden.",'_csrf-frontend':'".Yii::$app->request->getCsrfToken()."'},
     dataType:\"json\",
     success:function(data)
     {
      // console.log(data[0]);
...
obtenerProductoId();
  ...
  function agregarProductoArray(data)
  {
    encerarFactura();
    //console.log('{'+data+'}');
    data2='{'+data+'}';
    for (i=0;i<data2.length;i++){ 
        if (data[i]!=undefined){
            console.log('C: '+data[i].cantidad)
            if(data[i].cantidad>1){
                for (j=0;j<data[i].cantidad;j++){ 
                    obtenerProductoC(data[i])
