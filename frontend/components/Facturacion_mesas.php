...
class Facturacion_mesas extends Component
{
    private const MODULO='ORDENAR';

    public function getMesases($tipo,$array=true,$orderby,$limit,$all=true)
    {
        if ($all){
            $response= Mesas::find()->where(["isDeleted"=>0])->all();
        }else{
            $response= Mesas::find()->where(["isDeleted"=>0])->one();
        }
    }

    public function getMesas($id,$condicion=NULL,$itemsret=NULL)
    {
        $result=array();
        if ($id){
            $result= Mesas::find()->where(["id"=>$id])->one();
            if ($result)
            {
    ...
    {
        $clientes = Mesas::find()->where("isDeleted = 0")->orderBy(["nombre" => SORT_ASC])->all();
        //var_dump($clientes);
        $clientesArray=array();
        $cont=0;
        foreach ($clientes as $key => $value) {
            if ($cont==0){ $clientesArray[$cont]["value"]="Seleccione una Mesa"; $clientesArray[$cont]["id"]=-1; $cont++; }
    ...
    public function setStatus($id,$estatus)
    {
        $model = Mesas::find()->where(["isDeleted"=>0, "id"=>$id])->one();
        //var_dump($model);
        $model->estatusmesa=$estatus;
        $model->save();
    ...
    public function Editar($data)
    {
        ...
        if ($data):
            //$data = $usuario;
            $dataModel= Mesas::find()->where(["id"=>$data['id']])->one();
    ...
    public function Eliminar($id)
    {
        ...
        if ($id):
            //$data = $usuario;
            $dataModel= Mesas::find()->where(["id"=>$id])->one();
